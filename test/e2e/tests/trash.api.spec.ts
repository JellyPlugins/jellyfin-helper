...
test('a literal null body → clean 400 (never 500/NRE) on all three body-taking trash endpoints', async () => {
  // Existing tests post {} (a present-but-empty object) -> the field-blank branch ("TrashFolderPath is required.").
  for (const route of ['Trash/Relocate', 'Trash/CheckAccess', 'Trash/FoldersForPath']) {
    ...
    // Body is either the {Error:"Request body is required."} guard or a problem-details {title,status,errors} envelope - both are acceptable 400s; the point is that a null body neither 500s nor is silently accepted.
