...
if [ "$BUILD" -eq 1 ]; then
  ...
  # Only the plugin DLL + its non-Jellyfin runtime dependency are needed.
  ...
  [ -f "$PLUGIN_STAGE/Jellyfin.Plugin.JellyfinHelper.dll" ] || {
    echo "No staged plugin found. Run without --no-build first." >&2; exit 1; }
fi

# --- 2. fresh runtime dirs -------------------------------------------------
log "Resetting runtime state"
rm -rf "$RUNTIME/config" "$RUNTIME/cache" "$RUNTIME/media"
mkdir -p "$RUNTIME/config" "$RUNTIME/cache" "$RUNTIME/media/.gen"
cp "$E2E_DIR/fixtures/gen-media.sh" "$RUNTIME/media/.gen/gen-media.sh"

# On Linux/CI, run the container as the host user so /config is writable.
