...
- **Task settings explain themselves.** Every task dropdown in Settings now carries a short description of what the task does, shown on hover. Available in all dashboard languages.
...
- **Unit: 6204 total.**
- **End-to-end: 350 tests across 54 files.**
...
- **Other and Books columns in the Per-Library Breakdown.** The Overview table now shows an Other column per library for everything outside video, audio, subtitles, images and trickplay data (NFO files, text files and other sidecar files) plus a Books column whenever a book library exists, so every row now adds up exactly to its Total.
...
- **Unit: 6169 total.**
- **End-to-end: 341 tests across 54 files.**
...
- **Chart tooltips work reliably on touch.** On a phone or tablet, the first tap on the Trends growth chart or a Codecs breakdown ring could flash the tooltip and hide it again straight away. Tapping now shows and keeps the tooltip, and a second tap on the same Codecs ring segment closes it as before.
- **The save confirmation only appears after a real save.** Changing a setting and then setting it back to its original value briefly showed "All changes saved" even though nothing was saved. The confirmation now shows only after an actual save; reverting an edit simply clears the unsaved prompt.
