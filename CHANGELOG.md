...
- **Other and Books columns in the Per-Library Breakdown.** The Overview table now shows an Other column per library for everything outside video, audio, subtitles, images and trickplay data – NFO files, text files and .strm links – plus a Books column whenever a book library exists, so every row now adds up exactly to its Total.
