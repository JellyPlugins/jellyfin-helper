...
### Improved
- **Batch User Data Fetching (JF 12+ API)** - `WatchHistoryService` and `UserActivityInsightsService` now use `IUserDataManager.GetUserDataBatch(items, user)` to fetch user watch state for all library items in a single database call per user, replacing the previous per-item `GetUserData()` loop. On large libraries with multiple users this reduces the number of database roundtrips from `O(items × users)` to `O(users)`, cutting the runtime of recommendation and activity scans dramatically. Both services fall back to per-item lookups if the batch call throws, so behavior never regresses below the pre-batch baseline.
- **Batch People Lookup (JF 12+ API)** - `SimilarityComputer` now uses `ILibraryManager.GetPeopleNamesByItems(itemIds, personTypes)` to resolve actor/director names for all candidate items in a single call during the recommendation scoring hot path (~1000+ candidates per user), eliminating N per-candidate `GetPeople(item)` roundtrips. Falls back to the per-item `GetPeople(item)` path if the batch call throws, so behavior never regresses below the pre-batch baseline.
- **Shared `BatchFallbackHelper`** - Extracted the "try batch, fall back per-item" try/catch shape from all three batch call sites (`SimilarityComputer`, `WatchHistoryService`, `UserActivityInsightsService`) into `Services/Common/BatchFallbackHelper.cs`. The helper enforces a single contract for cancellation propagation, fatal-exception rethrow (OOM / stack overflow) and the graceful degrade-to-fallback path, so no future call site can silently drift on this behaviour again.
- **Cleanup: `IFileSystem` Alias** - Removed the `using IFileSystem = MediaBrowser.Model.IO.IFileSystem;` alias in `HelperCleanupTask.cs`. The codebase now cleanly references only one file-system abstraction where it matters.
...
### Tests
- Total: **2400 tests** (+82 vs. v2.1.0.5). New tests cover the JF 12 batch fallback paths (with distinct contracts for non-cancellation failures vs. `OperationCanceledException` in `SimilarityComputer`, `UserActivityInsightsService`, and `WatchHistoryService`), the new shared `BatchFallbackHelper` (12 tests locking down the cancellation-propagates / non-fatal-degrades / fatal-rethrows contract), the `IsEnabled(...)` CA1873 guard in `PluginLogService` (verifies no ILogger forwarding when the level is disabled), expanded `RecommendationPlaylistService` coverage (managed-name detection incl. numeric-suffix guard, side-effect ordering of create→delete, stale-playlist cleanup with zero recommendations, cancellation, and multi-user aggregation), and `UpdatePlaylists_WithSeriesRecommendations_ResolvesToEpisodes`.
