...
- **Other column in the Per-Library Breakdown.** The Overview table now shows an Other column per library for everything outside video, audio, subtitles, images and trickplay data – NFO files, text files, .strm links and eBooks included – so every row now adds up exactly to its Total.
