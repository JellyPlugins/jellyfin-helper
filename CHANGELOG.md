...
- **Smart Recommendation Engine** — ML-based per-user recommendation system (`Services/Recommendation/`) with four-tier scoring architecture:
  ...
  - `LearnedScoringStrategy` — gradient-descent ML with Z-score standardization, `ArrayPool`-based scoring, and per-feature weight importance logging (Debug level). Weights persisted to disk (JSON, schema v1).
  - `NeuralScoringStrategy` — three-hidden-layer MLP (28→32→16→8→1, 1601 params) with Adam optimizer, Xavier/Glorot initialization per layer, three-layer backpropagation, sigmoid output, early stopping with validation split, temporal sample weighting, weight clamping, `ReaderWriterLockSlim` thread-safety, `[ThreadStatic]` scratch buffers, input-gradient attribution for `ScoreWithExplanation()`, and per-feature importance logging (L2 norm, Debug level). Weights persisted to disk (JSON, schema v1).
...
- **Critic Rating Feature** — New `CriticRatingScore` scoring feature (28th feature) that normalizes Jellyfin's `CriticRating` (Rotten Tomatoes Tomatometer, 0–100%) to a 0–1 signal. Complements the existing community rating (audience score) with professional critic consensus. Defaults to 0.5 (neutral) when metadata is unavailable. Weight: 0.03.
- **Unknown-Genre Damping** — `SimilarityComputer.ComputeGenreSimilarity()` now reduces cosine similarity proportionally when a candidate contains genres the user has never watched. Damping factor 0.5 (moderate: "never watched" ≠ "dislikes"). Prevents items sharing one familiar genre plus several unknown genres from scoring as high as items where all genres are familiar.

### Changed
- **Weight Rebalancing** — Default scoring weights retuned: `CollaborativeScore` 0.11→0.09, `GenreCountNormalized` 0.02→0.01, `IsSeries` 0.04→0.01 (near-neutral, ML learns preference), `GenreUnderexposure` -0.08→-0.12, `GenreAffinityGap` -0.05→-0.08.
