...
internal static class ParentalRatingHelper
{
    ...
    internal static bool ShouldExclude(TmdbDiscoverItem candidate, int? maxParentalRating)
    {
        ...
        {
            return ShouldExcludeForChild(candidate);
        }
    ...
    /// <summary>
    ///     Applies the strict whitelist filter used for child accounts (FSK-6 and below).
    /// </summary>
    /// <param name="candidate">The TMDb discover item to check.</param>
    /// <returns>True if the item should be excluded for a child account, false otherwise.</returns>
    private static bool ShouldExcludeForChild(TmdbDiscoverItem candidate)
    {
