(function () {
    ...
    function renderStats(data) {
        ...
        html += '<div class="stat-card"><h3>' + T('movieVideoData', '🎬 Video Data — Movies') + '</h3>';
        html += '<p class="stat-value">' + formatBytes(data.TotalMovieVideoSize) + '</p>';
        var movieFiles = 0;
        for (var m = 0; m < data.Movies.length; m++) movieFiles += data.Movies[m].VideoFileCount;
        html += '<p class="stat-detail">' + movieFiles + ' ' + (movieFiles === 1 ? T('file', 'file') : T('files', 'files')) + ' ' + T('across', 'across') + ' ' + data.Movies.length + ' ' + T('libraries', 'libraries') + '</p>';
        html += '</div>';

        html += '<div class="stat-card"><h3>' + T('tvVideoData', '📺 Video Data — TV Shows') + '</h3>';
        html += '<p class="stat-value">' + formatBytes(data.TotalTvShowVideoSize) + '</p>';
        var tvFiles = 0;
        for (var t = 0; t < data.TvShows.length; t++) tvFiles += data.TvShows[t].VideoFileCount;
        html += '<p class="stat-detail">' + tvFiles + ' ' + (tvFiles === 1 ? T('episode', 'episode') : T('episodes', 'episodes')) + ' ' + T('across', 'across') + ' ' + data.TvShows.length + ' ' + T('libraries', 'libraries') + '</p>';
        html += '</div>';

        html += '<div class="stat-card"><h3>' + T('musicAudioData', '🎵 Music / Audio') + '</h3>';
        html += '<p class="stat-value">' + formatBytes(data.TotalMusicAudioSize) + '</p>';
        html += '<p class="stat-detail">' + data.TotalAudioFileCount + ' ' + (data.TotalAudioFileCount === 1 ? T('file', 'file') : T('files', 'files')) + '</p>';
        html += '</div>';

        html += '<div class="stat-card"><h3>' + T('trickplayData', '🖼️ Trickplay Data') + '</h3>';
        html += '<p class="stat-value">' + formatBytes(data.TotalTrickplaySize) + '</p>';
        var trickplayFolders = 0;
        for (var tp = 0; tp < data.Libraries.length; tp++) trickplayFolders += data.Libraries[tp].TrickplayFolderCount;
        html += '<p class="stat-detail">' + trickplayFolders + ' ' + (trickplayFolders === 1 ? T('folder', 'folder') : T('folders', 'folders')) + '</p>';
        html += '</div>';

        html += '<div class="stat-card"><h3>' + T('subtitleData', '📝 Subtitles') + '</h3>';
        html += '<p class="stat-value">' + formatBytes(data.TotalSubtitleSize) + '</p>';
        var subFiles = 0;
        for (var sb = 0; sb < data.Libraries.length; sb++) subFiles += data.Libraries[sb].SubtitleFileCount;
        html += '<p class="stat-detail">' + subFiles + ' ' + (subFiles === 1 ? T('file', 'file') : T('files', 'files')) + '</p>';
        html += '</div>';

        html += '<div class="stat-card"><h3>' + T('totalFiles', '📊 Total Files') + '</h3>';
        var totalMediaFiles = data.TotalVideoFileCount + data.TotalAudioFileCount;
        html += '<p class="stat-value">' + totalMediaFiles + ' ' + (totalMediaFiles === 1 ? T('mediaFile', 'media file') : T('mediaFiles', 'media files')) + '</p>';
        html += '<p class="stat-detail">' + data.TotalVideoFileCount + ' ' + T('video', 'video') + ', ' + data.TotalAudioFileCount + ' ' + T('audio', 'audio') + '</p>';
        html += '</div>';
        html += '</div>';
        ...
        html += '<div class="section-title">' + T('storageDistribution', 'Storage Distribution') + ' — <span style="color:#00a4dc;">' + formatBytes(grandTotal) + ' ' + T('total', 'Total') + '</span></div>';
        html += buildBarSegments(data);

        // Per-library breakdown table
        html += '<div class="section-title">' + T('perLibraryBreakdown', 'Per-Library Breakdown') + '</div>';
        html += '<div class="library-table-wrapper"><table class="library-table">';
        html += '<thead><tr>';
        html += '<th>' + T('library', 'Library') + '</th><th>' + T('type', 'Type') + '</th><th>' + T('video', 'Video') + '</th><th>' + T('audio', 'Audio') + '</th><th>' + T('subtitles', 'Subtitles') + '</th><th>' + T('images', 'Images') + '</th><th>' + T('trickplay', 'Trickplay') + '</th><th>' + T('total', 'Total') + '</th>';
        html += '</tr></thead><tbody>';

        for (var i = 0; i < data.Libraries.length; i++) {
            var lib = data.Libraries[i];
            html += '<tr>';
            html += '<td>' + lib.LibraryName + '</td>';
            html += '<td>' + getCollectionBadge(lib.CollectionType) + '</td>';
            html += '<td>' + formatBytes(lib.VideoSize) + '</td>';
            html += '<td>' + formatBytes(lib.AudioSize) + '</td>';
            html += '<td>' + formatBytes(lib.SubtitleSize) + '</td>';
            html += '<td>' + formatBytes(lib.ImageSize) + '</td>';
            html += '<td>' + formatBytes(lib.TrickplaySize) + '</td>';
            html += '<td><strong>' + formatBytes(lib.TotalSize) + '</strong></td>';
            html += '</tr>';
        }

        html += '</tbody></table></div>';
        html += '<div id="cleanup-stats-container"></div>';
        ...
        html += '<div class="chart-box"><h4>' + T('videoCodecs', '🎬 Video Codecs') + '</h4>';
        html += renderDonutChart(videoCodecs);
        html += '</div>';

        html += '<div class="chart-box"><h4>' + T('audioCodecs', '🎵 Audio Codecs') + '</h4>';
        html += renderDonutChart(audioCodecs);
        html += '</div>';

        html += '<div class="chart-box"><h4>' + T('containerFormats', '📦 Container Formats') + '</h4>';
        html += renderDonutChart(containers);
        html += '</div>';

        html += '<div class="chart-box"><h4>' + T('resolutions', '📐 Resolutions') + '</h4>';
        html += renderDonutChart(resolutions);
        html += '</div>';

        html += '</div>'; // charts-row
        html += '</div>'; // end codecs tab

        // === HEALTH TAB ===
        html += '<div class="tab-content" id="tab-health">';
        html += '<div class="section-title">' + T('healthChecks', 'Library Health Checks') + '</div>';
        html += renderHealthChecks(data);
        html += '</div>'; // end health tab

        // === TRENDS TAB (placeholder, filled async) ===
