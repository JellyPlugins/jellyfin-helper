...
public sealed class TrashServiceInternalHelpersTests
{
    ...
    [InlineData(null, 100)]
    [InlineData("", 100)]
    // A zero budget must not produce a single-char result, else callers would try to Directory.Move to
    // an over-budget path and hit IOException. Negative budgets come from GetMaxComponentSize when the
    // directory itself already exhausts the platform limit; the helper must degrade cleanly, not throw.
    [InlineData("anything", 0)]
    [InlineData("anything", -5)]
    public void TruncateToSize_EmptyOrNonPositiveBudget_ReturnsEmpty(string? value, int maxSize)
    {
        Assert.Equal(string.Empty, TrashService.TruncateToSize(value!, maxSize));
    }

    [Theory]
    [InlineData("short-name", 100)]
    // Boundary condition: the value exactly hits the budget. An off-by-one (< instead of <=) truncates here.
    [InlineData("abcde", 5)]
    public void TruncateToSize_ValueFitsBudget_ReturnsUnchanged(string value, int maxSize)
    {
        Assert.Equal(value, TrashService.TruncateToSize(value, maxSize));
    }
    ...
    // Bare or malformed input passes through untouched: anything shorter than "yyyyMMdd-HHmmss_" (16 chars)
    // cannot carry a prefix; 15 chars is the format length but has no trailing "_<name>"; a 16th char that
    // is not "_" or a well-formed-position underscore over an invalid timestamp both fail the pattern; and a
    // valid timestamp plus underscore with nothing after it hits the length guard. A naive substring approach
    // would produce garbage on several of these.
    [Theory]
    [InlineData("")]
    [InlineData("short")]
    [InlineData("20260601-120000")]
    [InlineData("20260601-1200000")]
    [InlineData("abcdefgh-ijklmn_original")]
    [InlineData("20260601-120000_")]
    public void ExtractOriginalName_InvalidOrBoundary_ReturnsInputUnchanged(string input)
    {
        Assert.Equal(input, TrashService.ExtractOriginalName(input));
    }
    ...
    // Rejected inputs: null, empty, the bare 15-char format length (the guard demands length + 1), an
    // out-of-range month, and a missing dash between date and time. All must fail and leave MinValue.
    [Theory]
    [InlineData(null)]
    [InlineData("")]
    [InlineData("20260601-120000")]
    [InlineData("20261301-000000_bad")]
    [InlineData("20260601 120000_bad")]
    public void TryParseTrashTimestamp_Invalid_ReturnsFalseWithMinValue(string? input)
    {
        var ok = TrashService.TryParseTrashTimestamp(input!, out var ts);
        Assert.False(ok);
