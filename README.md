...
| **🤖 Smart Recommendations** | ML-powered per-user recommendations with three-tier scoring (heuristic + learned + ensemble blend). Analyses watch history to build genre affinity profiles and scores unwatched candidates using 12 features including collaborative filtering, recency, ratings, completion tracking, and interaction terms. The ML model is retrained on every scheduled task run (default: weekly) using the latest watch data |
