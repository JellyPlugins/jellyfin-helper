...
| **🤖 Smart Recommendations** | ML-powered per-user recommendations using ensemble scoring (heuristic + learned + neural MLP blend). Analyses watch history to build genre, actor, director and studio affinity profiles, then scores unwatched candidates using 28 features: collaborative filtering, tag similarity, recency, community & critic ratings (Tomatometer), completion ratio labels, people/studio similarity, series progression, popularity, time-based affinity, genre exposure analysis, unknown-genre damping, and interaction features. Respects Jellyfin's per-user parental rating restrictions — children with restricted profiles only see age-appropriate recommendations. Incremental training runs only when TaskMode=Activate, with ranking-based evaluation (Precision@K, Recall@K, NDCG@K) and pre-computed collaborative filtering pipeline — pure C# implementation with zero external ML dependencies |
