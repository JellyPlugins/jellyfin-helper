...
| **🤖 Smart Recommendations** | ML-powered per-user recommendations with four-tier scoring (heuristic → learned → neural MLP → ensemble blend). Analyses watch history to build genre affinity profiles and scores unwatched candidates using 18 features including collaborative filtering, recency, ratings, completion tracking, people/studio similarity, series progression, popularity, day-of-week affinity, and interaction terms. The neural network (18→8→1 MLP with Adam optimizer) and linear model are retrained on every scheduled task run (default: weekly) using the latest watch data — pure C# implementation with zero external ML dependencies |
